...
public struct GeneratableMacro: MemberMacro, ExtensionMacro {
  public static func expansion(
    ...
    let props: [(name: String, schema: String, isRequired: Bool)] = variableDecls.flatMap {
      $0.bindings
    ...
    let propertyExprsString = props.map { "\"\\\"" + $0.name + "\\\": \" + " + $0.schema }.joined(
      separator: " + \",\" + ")
    let propertyExprsStringFinal =
      propertyExprsString.isEmpty ? emptyStringToken : propertyExprsString
    let requiredExprString = props.filter { $0.isRequired }.map { "\"\\\"" + $0.name + "\\\"\"" }
      .joined(separator: " + \",\" + ")
  ...
  private static func schema(
    for type: TypeSyntax, in declaration: some DeclGroupSyntax, context: some MacroExpansionContext
  ) -> String {
