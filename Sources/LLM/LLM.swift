...
public actor LLMCore {
  ...
  func setParameters(
    seed: UInt32? = nil, topK: Int32? = nil, topP: Float? = nil, temp: Float? = nil,
    repeatPenalty: Float? = nil, repetitionLookback: Int32? = nil
  ) {
    ...
    if seed != nil || topK != nil || topP != nil || temp != nil || repeatPenalty != nil
      || repetitionLookback != nil
    {
      recreateSampler()
    }
  }
  ...
  private func recreateSampler() {
    ...
    llama_sampler_chain_add(
      sampler!, llama_sampler_init_penalties(repetitionLookback, repeatPenalty, 0, 0))
  ...
  public init(
    model: Model, path: [CChar], seed: UInt32, topK: Int32, topP: Float, temp: Float,
    repeatPenalty: Float, repetitionLookback: Int32, maxTokenCount: Int
  ) throws {
  ...
  public func decode(_ token: Token, special: Bool = false) -> String {
    ...
    var actualLength = Int(
      llama_token_to_piece(vocab, token, &buffer, Int32(bufferLength), 0, special))
  ...
  func resetContext() {
    ...
    tokenBuffer.removeAll()
    shouldContinuePredicting = false
    // Full memory clear — llama_memory_seq_rm doesn't reset recurrent/hybrid
    // memory state, causing assertion failures on subsequent generations.
  ...
  func generateResponseStreamWithThinking(
    from input: String, thinking thinkingMode: ThinkingMode = .none
  ) -> (thinking: AsyncStream<String>, response: AsyncStream<String>) {
  ...
  public func generateWithConstraints(
    from input: String, jsonSchema: String, thinking: ThinkingMode = .suppressed
  ) throws -> String {
    debugLastGeneratedTokens = []
    guard prepareContext(for: input) else { throw LLMError.contextCreationFailed }

    guard let parsedSchema = parseJSONSchema(jsonSchema) else {
      throw LLMError.contextCreationFailed
    }
    if let sampler { llama_sampler_reset(sampler) }
    ...
    guard
      let originalSchema = try JSONSerialization.jsonObject(with: jsonSchema.data(using: .utf8)!)
        as? [String: Any]
    else {
      throw LLMError.contextCreationFailed
    }
    ...
    for (index, field) in parsedSchema.fields.enumerated() {
      ...
      try generateValueForField(
        field, into: &output, originalSchema: originalSchema, allowNull: !field.required)
  ...
  public func generateWithGrammar(
    ...
    llama_sampler_chain_add(
      grammarSampler, llama_sampler_init_penalties(repetitionLookback, repeatPenalty, 0, 0))
    ...
    // Custom generation loop that avoids double-accepting tokens.
    // `llama_sampler_sample` already calls `llama_sampler_accept` internally,
    // so we must NOT call `llama_sampler_accept` again (unlike `predictNextToken`
    // which does). Double-accepting crashes the grammar sampler because it
    // advances the state machine past the expected position.
    while shouldContinuePredicting && currentTokenCount < Int32(maxTokenCount) {
      let batchIndex = batch.n_tokens - 1
      let token = llama_sampler_sample(grammarSampler, context, batchIndex)

      guard token != endToken else { break }

      // Do NOT call llama_sampler_accept here — already done inside sample
  ...
  private func getValueStartTokensForField(_ field: SchemaField) -> Set<Token> {
    ...
    case .integer, .number:
      return Set(
        "0123456789-".compactMap { encode(String($0), shouldAddBOS: false, special: false).first })
    case .boolean:
  ...
  private func generateValueForField(
    _ field: SchemaField, into output: inout String, originalSchema: [String: Any],
  ...
  private func generateConstrainedString(into output: inout String, allowedValues: [String]) throws
  {
    ...
    while !candidateValues.isEmpty {
      let possibleNextTokens = Set(
        candidateValues.flatMap { value in
  ...
  private func generateObjectValue(into output: inout String, with schema: [String: Any]? = nil)
    throws
  {
    try addToken("{", to: &output)
  ...
  private func generateValueForFieldInfo(_ fieldInfo: [String: Any], into output: inout String)
    throws
  {
  ...
  private func findFieldSchemaInOriginal(_ fieldName: String, originalSchema: String) -> [String:
    Any]?
  {
    guard let schemaData = originalSchema.data(using: .utf8),
      ...
      let properties = schema["properties"] as? [String: Any],
      let fieldSchema = properties[fieldName] as? [String: Any]
    else {
      return nil
    }
    return fieldSchema
  ...
  private func findFieldSchema(_ fieldName: String, in originalSchema: [String: Any]) -> [String:
    Any]?
  {
  ...
  private func generateArrayValue(
    into output: inout String, ofType itemType: JSONFieldType,
    withItemSchema itemSchema: [String: Any]? = nil
  ) throws {
    try addToken("[", to: &output)
...
open class LLM: ObservableObject {
  ...
  public var preprocess:
    @Sendable (_ input: String, _ history: [Chat], _ thinking: ThinkingMode) -> String = {
      input, _, _ in return input
    }
  ...
  static func silenceLogging() {
    ...
    isLogSilenced = true
    let noopCallback:
      @convention(c) (ggml_log_level, UnsafePointer<CChar>?, UnsafeMutableRawPointer?) -> Void = {
        _, _, _ in
      }
  ...
  public convenience init?(
    ...
    historyLimit: Int = 8,
    maxTokenCount: Int32 = 2048,
    updateProgress: @Sendable @escaping (Double) -> Void = {
      print(String(format: "downloaded(%.2f%%)", $0 * 100))
    }
  ) async throws {
  ...
  private func setupThinkingTokens(from template: Template?) async {
    ...
    await core.setThinkingTokens(
      start: startTokens, end: endTokens, startMarker: startMarker, endMarker: endMarker)
  }
  ...
  open func recoverFromLengthy(
    _ input: borrowing String, to output: borrowing AsyncStream<String>.Continuation
  ) {
    output.yield("TL;DR")
  }
  ...
  public func respond(
    to input: String, thinking: ThinkingMode = .none,
  ...
  open func respond(to input: String, thinking: ThinkingMode = .none) async {
    ...
    let (thinkingStream, responseStream) = await core.generateResponseStreamWithThinking(
      from: processedInput, thinking: thinking)
    ...
    @Sendable (_ input: String, _ history: [Chat], _ thinking: ThinkingMode) -> String
  {
    return { [self] input, history, thinking in
      ...
      if thinking == .enabled, let start = thinkingStart?.stringValue {
        processed += start
      } else if thinking == .suppressed, let start = thinkingStart?.stringValue,
        let end = thinkingEnd?.stringValue
      {
        processed += start + end
      }
...
public struct HuggingFaceModel {
  ...
  public func download(
    to directory: URL = .documentsDirectory, as name: String? = nil,
  ...
  public static func tinyLLaMA(_ quantization: Quantization = .Q4_K_M, _ systemPrompt: String)
    -> HuggingFaceModel
  {
    HuggingFaceModel(
      "TheBloke/TinyLlama-1.1B-Chat-v1.0-GGUF", quantization, template: .chatML(systemPrompt))
  }
}
...
extension URL {
  ...
  fileprivate func downloadData(
    to destination: URL, _ updateProgress: @Sendable @escaping (Double) -> Void
  ) async throws {
...
extension String {
  package func matches(in content: String) throws -> [String] {
  ...
  package func hasMatch(in content: String) throws -> Bool {
    let pattern = try NSRegularExpression(pattern: self)
    let range = NSRange(location: 0, length: content.utf16.count)
    return pattern.firstMatch(in: content, range: range) != nil
  }
  package func firstMatch(in content: String) throws -> String? {
    let pattern = try NSRegularExpression(pattern: self)
    let range = NSRange(location: 0, length: content.utf16.count)
...
extension Character {
  var isValidStringCharacter: Bool {
    ...
    return isLetter || self == " " || isNumber || isLowercase || isUppercase
      || isASCII && isPunctuation || isASCII && isSymbol
  }
}
