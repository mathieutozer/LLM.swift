...
final class LLMTests {
  ...
  lazy var model = HuggingFaceModel(
    "unsloth/Qwen3-0.6B-GGUF", .Q4_K_M, template: .chatML(systemPrompt))
  let urlString =
  ...
  func testStructuredOutputWithSignedNumbers() async throws {
    ...
    let result = try await bot.respond(
      to:
        "Coldest temperature below zero you can think of in Celsius, preferabably zero Kelvin in Celsius",
      as: Temperature.self
    )
    ...
    if !hasGivenNegative {
      print(
        "the temperature was not negative but \(temperature.degreesInCelcius). but probably because model is not capable enough"
      )
    }
    #expect(!output.isEmpty)
  ...
  func testColorSchema() {
    ...
    let schema =
      try! JSONSerialization.jsonObject(with: Vegetable.jsonSchema.data(using: .utf8)!)
      as! [String: Any]
  ...
  func testStructuredOutputWithVegetable() async throws {
    let bot = try await LLM(from: model, seed: 1_605_617_885)!
  ...
  func testNestedGeneratableSchemas() throws {
    ...
    let locationSchema =
      try JSONSerialization.jsonObject(with: Location.jsonSchema.data(using: .utf8)!)
      as! [String: Any]
    ...
    let restaurantSchema =
      try JSONSerialization.jsonObject(with: Restaurant.jsonSchema.data(using: .utf8)!)
      as! [String: Any]
    ...
    // Test Garden schema
    let gardenSchema =
      try JSONSerialization.jsonObject(with: Garden.jsonSchema.data(using: .utf8)!)
      as! [String: Any]
  ...
  func testNestedGeneratableStruct() async throws {
    ...
    let result = try await bot.respond(
      to:
        "Create a restaurant with name, cuisine type, and location (latitude and longitude coordinates).",
      as: Restaurant.self
    )
  ...
  func testOptionalFieldSchema() throws {
    let profileSchema =
      try JSONSerialization.jsonObject(with: Profile.jsonSchema.data(using: .utf8)!)
      as! [String: Any]
  ...
  func testStructuredOutputWithOptionalFields() async throws {
    let bot = try await LLM(from: model, seed: 3_978_003_299)!
  ...
  func testNestedGeneratableStructures() async throws {
    ...
    let result = try await bot.respond(
      to:
        "Create a software development project with tasks, team members, and office location details. Project name should be short and clear.",
      as: Project.self
    )
